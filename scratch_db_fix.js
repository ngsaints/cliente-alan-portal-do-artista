...
const { db, appSettingsTable } = require("@workspace/db");
const { eq, or, like } = require("drizzle-orm");

async function run() {
  try {
    console.log("Starting database settings category repair...");

    // Update keys starting with landing_ to portal category
